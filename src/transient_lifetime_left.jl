...
function tau_transient_ensemble(
    ...
    # Stable solution
    stable_init = [2.,1.,0.,1.5,0.5]./[scale, scale, scale, 1., 1.]
    p_mod = deepcopy(p_orig)
    p_mod[12] = fw
    ds_mod = ContinuousDynamicalSystem(sys2, u1, p_mod)
...
transient_inits = u2_fw_crit[1:1000:end-1,:] # 100 samples
println("initializing ", size(transient_inits)[1], " trajectories on ghost attractor...")

fw_range = 0.77:0.0005:0.786
...
for (i, fw_transient) in enumerate(fw_range)
    ...
    τ_transients = tau_transient_ensemble(
        ...
        transient_bbox = ds_sol_bbox(u2_fw_crit, eps = 0.015), Tmax = 20000, Δt_sample = Δt_sample,
...
h5open("transients_lifetimes_left.h5", "w") do file
